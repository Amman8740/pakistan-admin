...
const InverterForm = ({category}: {category: string}) => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        setBrands(allBrands.filter((b) => b.category === "inverters"));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label className="block font-medium mb-1">Inverter Name</label>
        <select
          name="name"
          value={form.name}
          onChange={handleChange}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block font-medium mb-1">Price Range (Rs K)</label>
          <input
            type="text"
            name="priceRange"
